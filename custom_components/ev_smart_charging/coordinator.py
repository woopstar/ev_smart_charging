...
class EVSmartChargingCoordinator:
    ...
    async def turn_on_charging(self, state: bool = True):
        ...
        if state is True:
            ...
            if self.solar_charging:
                self.solar_charging.start_default_charging(
            ...
            if self.solar_charging:
                self.solar_charging.start_default_charging(0)
            if self.charger_switch.entity_id is not None:
                _LOGGER.debug(
    ...
    async def update_sensors(
        ...
        if default_charging_current_updated and self.auto_charging_state == STATE_ON:
